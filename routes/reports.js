...
router.get('/', optionalAuth, [
  ...
  if (lat && lng) {
    ...
    reports.forEach(report => {
      const distance = require('sequelize').literal(`(
        6371 * acos(
          cos(radians(${latFloat})) * cos(radians(${report.latitud})) * 
          cos(radians(${report.longitud}) - radians(${lngFloat})) + 
          sin(radians(${latFloat})) * sin(radians(${report.latitud}))
        )
      )`);
      report.dataValues.distancia = parseFloat(distance.val);
    });
  }

  const processedReports = reports.map(report => {
    const multimedia = (report.multimedia || []).map(file => ({
      ...file,
